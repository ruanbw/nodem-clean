...
| `--dry-run`       | —    | 只列出将被删除的目录，不删除   | `false`       |
| `--size`          | —    | 计算并显示各目录大小           | `false`       |
...
**删除后还能恢复吗？**
不能。`node_modules` 会被直接删除，不会进入回收站，请确认目录无误后再执行；也可以先用 `--dry-run` 预览。
